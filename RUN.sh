#---
# title: "Genotype Reconstruction From Pool Sequences (Haplo/Diploid polyandric species Hymenoptera)"
# author: "Sonia Eynard"
# date: "30/10/2021"
# PIPELINE FOR QUEEN GENETIC ANCESTRY ESTIMATION AND GENOTYPE RECONSTRUCTION
...
# full simulations: where queen genotypes, allele frequencies in the drone pool are drawn from Dirichlet distributions in which sub-species are in specific proportions. With these simulations we tested the impact of depth, population composition in sub-species and model fit. 
# simulations from real data: where males from a diversity panel (Wragg et al. 2021) were combined to create a 'queen' and a pool of drones and for which offspring were drawn from. Using real data it is posisble to simulate expected linkage disequilibrium and thus test our models for this parameters.
...
# input file version from March 2020 (genome version: HAV3.1, vcf version: 870 and 628)
#---

#####################################################################################################################################
### Pre-requisite slurm load modules ###
...
#####################################################################################################################################
### Data preparation ###
## Cleaning VCF from the diversity panel of Wragg et al. 2021 ##
...
do
	...
	do
		...
		do
			echo ${a} ${i} ${j}
			sbatch -J hom -W --mem=200G --wrap=" python .local/bin/genoqueen_hom ${dirin}/Mix_${a} depth${i}_${j}.txt count${i}_${j}.txt ${n_col_snpid} 1 sim_model${i}_${j} 100000"
		done
	done
done
